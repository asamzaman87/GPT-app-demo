 ...
 * 
 * Storage: PostgreSQL (production-ready)
 */
...
function getPool(): pg.Pool {
  if (!pool) {
    ...
    pool.on('error', (err) => {
      console.error('Unexpected error on idle MCP OAuth database client', err);
    });
...
/**
 * Initialize default client in database
 */
async function initializeDefaultClient(): Promise<void> {
  ...
  try {
    const existing = await pool.query(
      'SELECT client_id FROM google_calendar_mcp_clients WHERE client_id = $1',
      [clientId]
    );
    
    if (existing.rows.length === 0) {
      await pool.query(`
        INSERT INTO google_calendar_mcp_clients (
          client_id, client_secret, client_name, redirect_uris, grant_types, response_types, token_endpoint_auth_method
        ) VALUES ($1, $2, $3, $4, $5, $6, $7)
      `, [
        clientId,
        clientSecret,
        'Default Google Calendar MCP Client',
      ...
      console.log('Default MCP OAuth client initialized in database');
    }
  } catch (error) {
...
initializeDefaultClient().catch(err => {
  console.error('Failed to initialize default MCP client:', err);
});
...
export async function registerClient(request: ClientRegistrationRequest): Promise<ClientRegistrationResponse> {
  ...
  const redirect_uris = request.redirect_uris || ['https://chatgpt.com/aip/g-*/oauth/callback'];
  const grant_types = request.grant_types || ['authorization_code'];
  const response_types = request.response_types || ['code'];
  const token_endpoint_auth_method = request.token_endpoint_auth_method || 'client_secret_post';
  const client_name = request.client_name || 'Dynamic Client';
  
  await pool.query(`
    INSERT INTO google_calendar_mcp_clients (
      client_id, client_secret, client_name, redirect_uris, grant_types, response_types, token_endpoint_auth_method
    ) VALUES ($1, $2, $3, $4, $5, $6, $7)
  `, [clientId, clientSecret, client_name, redirect_uris, grant_types, response_types, token_endpoint_auth_method]);
...
export async function getClient(clientId: string): Promise<RegisteredClient | undefined> {
  ...
  try {
    const result = await pool.query(
      'SELECT * FROM google_calendar_mcp_clients WHERE client_id = $1',
      [clientId]
    );
...
export async function generateAuthorizationCode(
  ...
  await pool.query(`
    INSERT INTO google_calendar_mcp_auth_codes (
      code, client_id, redirect_uri, code_challenge, code_challenge_method, expires_at, scope, resource
    ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
  `, [code, clientId, redirectUri, codeChallenge, codeChallengeMethod, expiresAt, scope, resource]);
...
export async function validateAuthorizationCode(
  ...
  try {
    const result = await pool.query(
      'SELECT * FROM google_calendar_mcp_auth_codes WHERE code = $1',
      [code]
    );
    ...
    if (new Date() > new Date(authCode.expires_at)) {
      await pool.query('DELETE FROM google_calendar_mcp_auth_codes WHERE code = $1', [code]);
...
export async function generateTokenPair(
  ...
  try {
    ...
    await pool.query(`
      INSERT INTO google_calendar_mcp_access_tokens (
        token, client_id, expires_at, scope, resource, refresh_token
      ) VALUES ($1, $2, $3, $4, $5, $6)
    `, [accessToken, clientId, accessExpiresAt, scope, resource, refreshToken]);
    
    // Store refresh token with link to access token
    await pool.query(`
      INSERT INTO google_calendar_mcp_refresh_tokens (
        token, client_id, expires_at, scope, resource, access_token
      ) VALUES ($1, $2, $3, $4, $5, $6)
    `, [refreshToken, clientId, refreshExpiresAt, scope, resource, accessToken]);
...
export async function validateAccessToken(token: string): Promise<boolean> {
  ...
  try {
    const result = await pool.query(
      'SELECT expires_at FROM google_calendar_mcp_access_tokens WHERE token = $1',
      [token]
    );
    ...
    if (new Date() > expiresAt) {
      await pool.query('DELETE FROM google_calendar_mcp_access_tokens WHERE token = $1', [token]);
      return false;
    }
...
export async function validateRefreshToken(token: string): Promise<{ 
  ...
  try {
    const result = await pool.query(
      'SELECT * FROM google_calendar_mcp_refresh_tokens WHERE token = $1',
      [token]
    );
    ...
    if (new Date() > expiresAt) {
      await pool.query('DELETE FROM google_calendar_mcp_refresh_tokens WHERE token = $1', [token]);
...
export async function revokeRefreshToken(token: string): Promise<void> {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT access_token FROM google_calendar_mcp_refresh_tokens WHERE token = $1',
      [token]
    );
    ...
    if (result.rows.length > 0 && result.rows[0].access_token) {
      ...
      await pool.query(
        'DELETE FROM google_calendar_mcp_access_tokens WHERE token = $1',
...
async function cleanupExpiredTokens(): Promise<void> {
  ...
  try {
    ...
    await pool.query(
      'DELETE FROM google_calendar_mcp_access_tokens WHERE expires_at < $1',
      [now]
    );
    
    // Clean up expired refresh tokens
    await pool.query(
      'DELETE FROM google_calendar_mcp_refresh_tokens WHERE expires_at < $1',
      [now]
    );
    
    // Clean up expired auth codes
    await pool.query(
      'DELETE FROM google_calendar_mcp_auth_codes WHERE expires_at < $1',
      [now]
    );
  } catch (error) {
